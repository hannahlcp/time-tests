...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            (times.time_range("2010-01-13 10:00:00", "2010-01-13 12:00:00")),
            ([]),
            id="no overlap",
        ),
        pytest.param(
        ...
        pytest.param(
            times.time_range("2025-06-25 00:00:00", "2025-06-25 00:10:00", 2, 60),
            times.time_range("2025-06-25 00:05:00", "2025-06-25 00:15:00", 2, 60),
            ([("2025-06-25 00:05:30", "2025-06-25 00:09:30")]),
            id="several overlapping",
        ),
    ],
)
def test_times_notoverlapping_parametrize(time_range_1, time_range_2, expected):
...
def test_times_several_overlapping2():
    ...
    result = times.compute_overlap_time(t0, t1)
    expected = [
        ("2025-06-25 00:05:30", "2025-06-25 00:07:15"),
        ("2025-06-25 00:08:15", "2025-06-25 00:10:00"),
    ]
    assert result == expected
...
def test_interval_larger_than_range():
    with raises(
        ValueError,
        match=r"Gap between intervals must be less than the total time range",
    ):
        times.time_range("2025-06-25 00:00:00", "2025-06-25 00:00:10", 2, 20)


# To generate coverage report, run on terminal:
